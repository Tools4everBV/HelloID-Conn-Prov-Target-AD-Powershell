...
if(-Not($dryRun -eq $True)) {
    try{
        ...
        if(($ADUser | Measure-Object).Count -ne 0) {
            ...
            Write-Verbose -Verbose "Removing user $aRef from group $($pRef.Id)"
            Remove-ADGroupMember -Identity $pRef.Id -Members $aRef -Confirm:$False
...
$auditLogs.Add([PSCustomObject]@{
    # Action = "RevokeMembership"; Optionally specify a different action for this audit log
    Message = "Permission $($pRef.Id) removed from account $($aRef)";
    IsError = $False;
});
