...
if(-Not($dryRun -eq $True)) {
    try{
        ...
        if(($ADUser | Measure-Object).Count -ne 0) {
            ...
            Write-Verbose -Verbose "Disabling account"
            Disable-ADAccount -Identity $ADUser.ObjectGUID
            Write-Verbose -Verbose "Move account to $($c.disableOU)"
            Move-ADObject -Identity $ADUser.ObjectGUID -TargetPath "$($c.disableOU)"
